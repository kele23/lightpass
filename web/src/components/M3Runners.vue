...
let tableData = ref([]);
tableData.value = await dbManager.getAllRunners(raceId);

/////////////////////////////////////////////////////
async function createRunner(event) {
    const data = { ...formToJSON(event.target), race: raceId };
    await dbManager.createRunner(data);
...
async function uploadCsv(event) {
    ...
    await dbManager.cleanRunners(raceId);
    ...
    for (const row of rows) {
        await dbManager.createRunner({ ...row, id: null, race: raceId });
    }
...
<template>
    <div class="w-full items-start lg:flex">
        ...
        <div class="w-full shrink-0 grow-0 lg:w-80">
            <form class="mb-4 rounded-lg bg-white shadow" @submit.prevent="createRunner">
                <div class="px-6 py-4">
                    <div class="relative mt-6">
                        <div class="relative flex items-center justify-between text-base font-semibold leading-5">
                            <span class="text-gray-500"> Nuovo Corridore </span>
                            <button
            ...
            <form class="mb-4 rounded-lg bg-white shadow" @submit.prevent="uploadCsv">
                <div class="px-6 py-4">
                    <div class="relative mt-6">
                        <div class="relative flex items-center justify-between text-base font-semibold leading-5">
                            <span class="text-gray-500"> Carica Corridori </span>
                            <button
                                class="text-md flex items-center rounded-full bg-white p-2 shadow hover:text-gray-700"
                                title="Clear"
                                type="reset"
